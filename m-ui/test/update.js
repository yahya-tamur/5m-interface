...
const status_re = /MachineStatus: (\S\w*)\r\nMoveMode: (\S\w*).*CurrentFile: (|\S\w*)\r\n/s
...
if (printer_ip_match = location.search.match(/[&?]printer-ip=([^&]+)/)) {
    ...
    document.getElementById('stream').src = `http://${printer_ip}:8080/?action=stream`;
    console.log(printer_ip);
}


const fill_data = () => {
    fetch(`./temp?printer-ip=${printer_ip}`).then(resp => resp.text()).then(text => {
    ...
    fetch(`./status?printer-ip=${printer_ip}`).then(resp => resp.text()).then(text => {
    ...
    fetch(`./progress?printer-ip=${printer_ip}`).then(resp => resp.text()).then(text => {
...
slider.addEventListener("change", (e) => {
    ...
    if (value > 0) {
        interval = setInterval(() => {fill_data();}, value);
    } else {
        interval = 0;
        // show button
    }
...
slider.addEventListener("input", (e) => {
    ...
    document.getElementById('sliderLabel').innerText = `${seconds} second${seconds == 1? '': 's'}`;
...
pause_alert.addEventListener('close', () => {
    if (pause_alert.returnValue == 'ok') {
        fetch(`./pause?printer-ip=${printer_ip}`).then(resp => {
            setTimeout(() => {pause_confirm.close();}, 2000);
            pause_confirm.show();
        }).catch((e) => {
            document.getElementById('pause-confirm-form').innerText = "Couldn't send pause signal.";
            pause_confirm.show();
        });
